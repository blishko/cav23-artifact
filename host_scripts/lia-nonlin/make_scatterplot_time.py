...
if __name__ == '__main__':
    if len(sys.argv) != 6:
        print("%s -- create scatter plot gnuplot scripts using tex driver" % sys.argv[0])
        print("Usage: %s <x-res> <y-res> <x-label> <y-label> <output>" % sys.argv[0])
        print("<x-res> and <y-res> are the result files for horizontal")
        print("and vertical axis.  The results files should consist of")
        print("lines of the form")
        print("")
        print("<name> <result> <time>")
        print("")
        print("where <name> identifies uniquely a formula, <result> is")
        print("MEMOUT, TIMEOUT, UNKNOWN, SAT or UNSAT, and <time> is a")
        print("floating point number.")
        print("<x-label> and <y-label> are the axis labels, and <output>")
        print("is a tex file where gnuplot will place its output.  The")
        print("script assumes %d seconds time out" % to)
    ...
    try:
        ...
        sys.exit(1)
    max_x = max([x_res[x][1] for x in x_res])
    max_y = max([y_res[x][1] for x in y_res])
    ...
    solved_x = len([x for x in list(x_res.keys()) if x_res[x][1] >= 0])
    solved_y = len([x for x in list(y_res.keys()) if y_res[x][1] >= 0])
    ...
    print('set term postscript eps color colortext')
    print('set output "%s"' % output)
    print('set size square')
    print('set lmargin 1')
    #print('set lmargin at screen 0.1')
    print('set xlabel "%s"' % sys.argv[3])
    print('set ylabel "%s" offset 2,0' % sys.argv[4])
    if (use_log):
    ...
    print('set arrow from %f, graph 0 to graph 1.00, graph -.10 backhead lt 2' % bnd)
    print('set label "t/o" at graph .95, graph -0.15')
    print('set arrow from %f, graph 0 to graph 1.03, graph -.05 backhead lt 2' % bnd2)
    print('set label "m/o" at graph 1.02, graph -0.07')
    print('plot x title "" lc "black", "-" title "" with point pointtype 2 lc "red" lw 2, "-" title "" with points pointtype 4 lc "forest-green" lw 2, "-" title "" with points pointtype 3 lc "black", "-" title "" with points pointtype 5 lc 1')
